...
export default class MovieDetails extends Component {
  ...
  async componentDidMount() {
    ...
    const response = await axios.get(
      `${URL}${movieId}?api_key=${KEY}&language=en-US`,
    );
    const responseCast = await axios.get(
      `${URL}${movieId}/credits?api_key=${KEY}&language=en-US`,
    );
    const responseReviews = await axios.get(
      `${URL}${movieId}/reviews?api_key=${KEY}&language=en-US`,
    );
      ...
      <>
        ...
        <Suspense fallback={<h1>Load...</h1>}>
          <Route
            ...
            render={props => cast && <Cast {...props} cast={cast} />}
          />
